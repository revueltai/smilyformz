...
-- Policy to allow authenticated users to insert their own game sessions
CREATE POLICY "Users can insert their own game sessions" ON game_sessions
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

-- Policy to allow users to view their own game sessions
CREATE POLICY "Users can view their own game sessions" ON game_sessions
  FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

-- Policy to allow users to update their own game sessions (if needed)
...
-- Policy to allow users to delete their own game sessions (if needed)
...
-- Policy to allow all authenticated users to view global rankings
CREATE POLICY "Anyone can view global rankings" ON global_ranking
  FOR SELECT
  TO authenticated
  USING (true);

-- Policy to allow users to insert their own ranking entries
CREATE POLICY "Users can insert their own ranking entries" ON global_ranking
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

-- Policy to allow users to update their own ranking entries
